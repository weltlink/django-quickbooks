...
from django_quickbooks.signals import customer_created, customer_updated, qb_task_create

RealmModel = get_realm_model()


@receiver(customer_created)
def create_qbd_customer(sender, qbd_model_mixin_obj, schema_name, *args, **kwargs):
    qb_task_create.send(
...
def update_qbd_customer(sender, qbd_model_mixin_obj, schema_name, *args, **kwargs):
    qb_task_create.send(
        sender=qbd_model_mixin_obj.__class__,
